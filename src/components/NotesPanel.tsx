...
import { GripHorizontalIcon, PaletteIcon, PlusIcon, SearchIcon, ToggleLeftIcon, ToggleRightIcon, TrashIcon, XIcon } from './Icons'
...
function NoteEditor({ note, onClose, onUpdate, onDelete, onCycleColor }: { note: Note; onClose(): void; onUpdate(id: string, patch: Partial<Note>): void; onDelete(id: string): void; onCycleColor(id: string): void }) {
  ...
  const controls = [
    ['bold', 'B', () => runCommand('bold')], ['italic', 'I', () => runCommand('italic')], ['underline', 'U', () => runCommand('underline')], ['strike', 'S', () => runCommand('strikeThrough')],
    ['h2', 'H2', () => runCommand('formatBlock', 'H2')], ['bullet', '•', () => runCommand('insertUnorderedList')], ['ordered', '1.', () => runCommand('insertOrderedList')], ['quote', '“', () => runCommand('formatBlock', 'BLOCKQUOTE')], ['code', '</>', () => runCommand('formatBlock', 'PRE')], ['clear', '⌫', () => { runCommand('removeFormat'); runCommand('formatBlock', 'DIV') }],
  ] as const

  return <section ref={frameRef} onMouseUp={rememberSize} className={`fixed z-50 min-h-[120px] min-w-[280px] max-w-[90vw] overflow-hidden rounded-2xl border shadow-2xl ${isColor ? palette.note : 'bg-white/90 text-slate-900 backdrop-saturate-200 backdrop-blur-xl'}`} style={{ left: position.x, top: position.y, width: note.size?.width ?? 420, height: collapsed ? 'auto' : note.size?.height ?? 520, resize: collapsed ? 'none' : 'both' }}>
    <header onPointerDown={startDrag} className="flex cursor-move items-center justify-between gap-2 px-4 py-3"><div className="flex min-w-0 flex-1 items-center gap-2 text-sm font-semibold"><GripHorizontalIcon className={`size-4 shrink-0 ${palette.toolbar}`} /><input value={note.title} onChange={event => onUpdate(note.id, { title: event.target.value })} placeholder="Untitled" className="w-full min-w-0 bg-transparent text-sm font-semibold text-slate-900 outline-none placeholder:text-slate-500" /></div><div className="flex shrink-0 items-center gap-1"><button type="button" onClick={() => setCollapsed(value => !value)} className="inline-flex size-8 items-center justify-center rounded-full border border-black/10 bg-white/40 text-slate-700 transition hover:bg-white/70" aria-label={collapsed ? 'Mở rộng note' : 'Thu gọn note'} title={collapsed ? 'Mở rộng note' : 'Thu gọn note'}>{collapsed ? '▾' : '▴'}</button><button type="button" onClick={() => onUpdate(note.id, { theme: isColor ? 'blur' : 'color' })} className="inline-flex size-8 items-center justify-center rounded-full border border-black/10 bg-white/40 text-slate-700 transition hover:bg-white/70" aria-label="Đổi nền note" title="Đổi nền note">{isColor ? <ToggleLeftIcon className="size-4" /> : <ToggleRightIcon className="size-4" />}</button>{isColor && <button type="button" onClick={() => onCycleColor(note.id)} className="inline-flex size-8 items-center justify-center rounded-full border border-black/10 bg-white/40 text-slate-700 transition hover:bg-white/70" aria-label="Đổi màu note" title="Đổi màu note"><PaletteIcon className="size-4" /></button>}<button type="button" onClick={() => onDelete(note.id)} className="inline-flex size-8 items-center justify-center rounded-full border border-black/10 bg-white/40 text-slate-700 transition hover:bg-white/70" aria-label="Xóa note" title="Xóa note"><TrashIcon className="size-4" /></button><button type="button" onClick={onClose} className="inline-flex size-8 items-center justify-center rounded-full border border-black/10 bg-white/40 text-slate-700 transition hover:bg-white/70" aria-label="Đóng note" title="Đóng note"><XIcon className="size-4" /></button></div></header>
    {!collapsed && <div className="h-[calc(100%-3.25rem)] px-4 pb-4"><div className="note-editor flex h-full flex-col rounded-xl rounded-t-none border border-black/10 bg-white/60"><div className="flex flex-wrap items-center gap-1 border-b border-black/10 bg-white/60 px-2 py-2">{controls.map(([key, label, onClick]) => <button key={key} type="button" onMouseDown={event => event.preventDefault()} onClick={onClick} className="h-7 min-w-[28px] rounded-md border border-black/10 bg-white/30 px-2 text-xs font-semibold text-slate-700 transition hover:bg-white/70" aria-label={key}>{label}</button>)}</div><div ref={editorRef} contentEditable suppressContentEditableWarning onInput={saveContent} onKeyUp={() => setToolbarVersion(version => version + 1)} className="min-h-[220px] flex-1 overflow-auto px-3 py-2 text-sm text-slate-900 outline-none" /></div></div>}
  </section>
}
